--------program for automatic processing----------------------------------------------------
-------------CONFIG-------------------------------------------------------------------------
...
local processing_unit_type = "minecraft:furnace" --type of unit to process with
...
local function main(term)
    ...
    log = require("log.lua").create_log(term.current())
    ...
    local furnaces = {peripheral.find(processing_unit_type)}
    for k,v in pairs(furnaces) do
        log(peripheral.getName(v),log.update)
    end
    log("found "..#furnaces.." units",log.success)
    log("")
